% CSE426 Final Project - Preprocess & Extract Moment & Pixel features
...
MomentFeature_A_RMS = sqrt(sum(MomentFeature.A.^2,1)/1000);

% use normalized factors from training data
MomentFeatureNum = length(MomentFeature.A(1,:));
for SetNum = 1:4
    for rr = 1:MomentFeatureNum
        MomentFeature.(dataset{SetNum})(:,rr) = MomentFeature.(dataset{SetNum})(:,rr)/MomentFeature_A_RMS(1,rr);
    end
end

%% Expand the feature matrix
for SetNum = 1:4
    Feature.(dataset{SetNum}) = [PixelFeature.(dataset{SetNum}) MomentFeature.(dataset{SetNum})];
end
